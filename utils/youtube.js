...
export async function getCurrentStream() {
  const liveBroadcastsResponse = await tryAPIRequest("https://www.googleapis.com/youtube/v3/liveBroadcasts?broadcastStatus=active&part=id,status,snippet");
  for (let liveBroadcastItem of liveBroadcastsResponse.items) {
    console.log(liveBroadcastItem);
    if (liveBroadcastItem.status.lifeCycleStatus === "live") {
      return {
        ...
        "privacyStatus" : liveBroadcastItem.status.privacyStatus
      }
    }
  }
  return null;
